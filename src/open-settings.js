...
/**
 * Find the settings tab Roam creates for Roam Caret.
 *
 * Developer extensions are displayed as "Roam Caret (dev)" even though
 * the registered tabTitle is still "Roam Caret", so prefer that exact tab
 * when both the Depot and developer builds are present.
 */
export function findRoamCaretSettingsTab(documentLike) {
...
export function selectRoamCaretSettingsTab(documentLike) {
...
/**
 * Open Roam Depot and focus Roam Caret's extension settings.
 *
 ...
 */
export async function openRoamCaretSettings({
